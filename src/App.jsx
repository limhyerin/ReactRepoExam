...
function App() {
  ...
  const filterByAge = (minAge) => {
    // 여기에 코드를 작성하세요.
      const newAge = students.filter(student => student.age >= minAge).map((item) => {
        return newAge
      })
  ...
  const filterByGrade = (grade) => {
    // 여기에 코드를 작성하세요.
    const newgrade = students.filter(student => student.grade >= grade).map((item) => {
      return newgrade
    })
  };
  ...
  const resetFilter = () => {
    // 여기에 코드를 작성하세요.
    //초기화
    const newReset = students.filter(student => student.age >= minAge).map((item) => {
      return 
    })
  };
